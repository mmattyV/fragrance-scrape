...
def get_perfume_links(designer_url):
    ...
    response.raise_for_status()  # Agrega esta línea para lanzar excepciones HTTP
...
# Inicializar el archivo de links de perfumes con el encabezado si no existe
if not os.path.exists('perfume_links_ES.txt'):
    with open('perfume_links_ES.txt', 'w') as file:
...
for i in range(current_index, len(designer_links)):
    ...
    try:
        ...
        save_perfume_links('perfume_links_ES.txt', {designer_name: perfume_links[designer_name]})
    except Exception as e:
        print(f"Error al procesar {designer_url}: {e}")
